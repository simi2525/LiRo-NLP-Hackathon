...
if __name__ == '__main__':
    ...
    dataset["train"] = dataset["train"].select(list(range(40000000)))
    ...
    # per_label_counts = [x*1000000 for x in per_label_counts]
    ...
    checkpoint_callback_hours = ModelCheckpoint(dirpath="checkpoints_canine_bert_2hours",every_n_train_steps=None,every_n_epochs=None, train_time_interval=timedelta(hours=2), save_top_k=6, monitor="step")
    ...
    model = DiacCanineBertTokenClassification.load_from_checkpoint('checkpoints2/epoch=1-step=179687.ckpt', strict=True, num_labels=len(utils.labels), per_label_weights=per_label_weights, lr=LR)
    ...
    trainer = Trainer(accelerator='gpu',precision='bf16', amp_backend="native",devices=2, logger=wandb_logger, callbacks=[checkpoint_callback,checkpoint_callback_hours], max_epochs=EPOCHS, accumulate_grad_batches=1)
    
    # trainer.fit(model, train_dataloader, test_dataloader)
    trainer.fit(model, train_dataloader, test_dataloader, ckpt_path='checkpoints2/epoch=1-step=179687.ckpt')